...
- ~~有大佬说是只要使用了dynamic_cast就一定会被推迟到运行时。除非是毫无关系的转换会在编译时就被拒绝。~~
  - ~~也就是说只要是合法的转换，无论是上行还是下行转换，dynamic_cast都依赖于虚函数表头的RTTI信息。所以都是运行时。~~
...
- 也就是说，如果加了explicit，那么我们必须用显式类型转换。比如C风格cast或c++风格cast或显式调用构造函数进行直接初始化。
...
- 在析构函数中是不可以抛出异常的。原因在《More Effective C++》中提到两个：
