...
这个东西的核心就是**刷新编译器的“类型视角”**：**告诉它这里是新构造出来的对象**，也就是强制编译器 **重新加载指针指向对象**；

> If a new object is created in the storage occupied by an existing object of the same type, or a type compatible with the new one, and the value of the pointer is reused without `std::launder`, the behavior is undefined.
