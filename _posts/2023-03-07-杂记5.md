...
我们在ADL中的不触发ADL的第三点中提到了CPO的关键要素。CPO本质上，就是一个**能够正确解决复杂定制问题的functor对象**。为了解决这个问题，我们需要屏蔽掉ADL调用。也就是说，我们把ADL的两步式（先using引入，再非限定调用）放到了定制点对象内部进行。而在外部我们并不使用ADL。
