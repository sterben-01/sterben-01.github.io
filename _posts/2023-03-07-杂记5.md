...
- 继承自抽象基类，比如：
  ...
  - 注意这里不是继承自`std::allocator`。也不应该这样做。原因见[这里](https://stackoverflow.com/questions/21081796/why-not-to-inherit-from-stdallocator)
...
不需要deallocate如何理解呢？一般情况下，通常只针对`monotonic_buffer_resource`因为它的`deallocate`是无作用。只有在析构的时候才会把资源交还给上游分配器。当然了，标准库提供的所有内存资源都符合RAII特性，在析构的时候把内存交还给上游分配器。所以这里不使用`deallocate`也是可以的。一定要记住，所有解分配`deallocate`都不会调用元素的析构函数。
