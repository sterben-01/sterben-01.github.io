...
- 编译器会从函数的（主）模板声明（declaration）中合成（synthesize）一个函数声明（declaration）
...
-   **！！！注意，在有非模板和主模板重载的时候，只有非模板和主模板重载参与重载决议。特化并不是重载，因此此时不受考虑。除非在重载决议选择最佳匹配的主函数模板后，才检验它的特化以查看最佳匹配者。**
...
- **函数的全特化是非常不好的设计。全特化提供的是一个替换的定义，而不是一个替换的声明。在调用函数模板的时点，该调用已经完全基于函数模板而完成解析了。所以：**
  - 无法在没有主模板的时候对函数模板进行全特化。
    - **因为全特化的函数没有新的mangled的名字。也就是全特化函数没有独立名字。**
...
- 针对第一个`getTypeName`函数调用，过程是这样的：
  ...
  - 在一开始，重载解析会考虑 (1) 和 (2)。第三个全特化版本确实不参与重载决议。
  ...
  - 但是随后，我们发现针对我们合成出来的`T`为`int`类型的函数，还有一个似乎可以比较一下。因为(3)此时被认为是(2)的全特化。而且此时两个函数都是模板函数。所以这时候（3）被加入到了重载决议中。
  - 此时和（2）相比，（3）的类型更为特化，所以此时选择了（3）。
