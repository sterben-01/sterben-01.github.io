...
- 为啥`foo0`和`foo2`的第二个都不行呢？因为参数类型推导中不能有类型转换。但是`foo`和`foo1`都是非模板函数，或者说，参数不涉及类型转换。比如`foo3`。这样的都可以。

  - 因为，`foo0`的参数类型涉及到推导的部分应该是`typewrapper`模板的模板参数类型。所以传入的参数会被推导为`tmp`类型。但是这个`foo0`函数的参数本身的类型应该是`typewrapper<T>`类型，两者类型不匹配。入参的类型被推导为`tmp`，但是函数要求的参数类型却是`typewrapper<tmp>`。

  - foo2同理。`foo2`的参数类型涉及到推导的部分应该是`std::function`模板的模板参数类型。所以传入的参数会被推导为`int, int, int`类型。但是这个`foo2`函数的参数本身的类型应该是`function<void(Args...)>`类型，两者类型不匹配。入参的类型被推导为`int,int,int`，但是函数要求的参数类型却是`function<void(int, int, int)>`。

  - 至于`foo3`为什么可以，因为`foo3`的入参可以被推导为任何可能的类型，不涉及到需要类型转换的部分。比如如果传入函数指针，则会被推导为`void (*)(int, int, int)`。如果传入`std::function`对象，则会被推导为`std::function<void(int, int, int)>`
...
这一章是核心，也是我觉得不太好理解的部分：也就是比`T`复杂得多的参数化类型也可以匹配一个给定的实参类型。
