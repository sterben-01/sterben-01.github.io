...
**一句话：push_back总是先创建对象，然后拷贝/移动到容器。emplace则可以选择直接在容器内构造对象。所以只有当我们想原地构造（直接放入容器 （直接传入构造函数参数调用构造函数来直接构建对象））的时候选用emplace才是最佳方法。如果是想要移入一个已存在的对象，则没有区别。因为二者遇到右值对象的时候都会调用移动构造。**
