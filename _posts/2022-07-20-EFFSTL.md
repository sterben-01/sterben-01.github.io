...
- `emplace_back()`在容器尾部添加一个元素时，可以直接传入构造函数需要的参数来直接在容器内原地构造。因为`emplace_back`使用了完美转发。也就时使用`emplace_back()`可以不显式调用对象的构造函数来避免一次额外的构造。**就是把所需参数完美转发至元素类型的构造函数。**
- 
  - 具体触发移动构造或拷贝构造的规则
...
- 使用`emplace_back`传入`pair`的时候不要忘记，`emplace_back`是原地构造。就是把所需参数完美转发至元素类型的构造函数。
...
## unordered_map 哈希表没有自定义排序。只有自定义哈希和自定义等比。想使用自定义的键类型，必须实现hash函数和等比函数。
