...
- 它能**原子地**检测对象是否有效。**如果对象存在，`lock()`函数返回一个指向共享对象的`shared_ptr`(引用计数会增1)，否则返回一个空`shared_ptr`。所以`lock()`并不是简单的判断。
...
- `weak_ptr`的大小是两个指针。**推测原因：**第一个指针指向的是对象，第二个指针指向的是控制块。因为`weak_ptr`和`shared_ptr`，继承自同一个基类
...
所以如果自定义删除器传入一个函数指针，因为不是空类，会被匹配到第二个模板。所以会把函数指针在`_Ty1 _Myval1;`这里存下来。就多了一个指针。
