...
- **针对条件变量和生产者消费者模型到底应该用几个条件变量的问题，比较看情况。有可能生产者和消费者各有一个，也可能同时使用一个。为什么呢？**
  - ~~假设现在有俩生产者线程和俩消费者线程。四个共用一个条件变量。这时候情况是 1个生产者和俩消费者都在`wait`。此时单独的抢到锁的生产者生产完毕，这时候`notify`不一定会`notify`消费者还是生产者。如果此时`notify`到了另一个生产者，那么消费者还在等。有可能出现一直是两个生产者在生产，消费者一直没有被`notify`到的情况（也就是消费者一直抢不到锁）。所以还是要依照情况。如果是各维护一个，那么生产者`notify`的一定是消费者。消费者`notify`的一定是生产者。不会出现循环情况。~~
  - 
