...
因为我们在`wait`函数返回之前，当前进程必须执行 **拿到锁-->加锁并唤醒** 这两步。加锁并唤醒是原子的，但是并不一定能拿得到这个锁。假如我们消费者1在等待，然后生产者生产完毕，通知消费者。假如这个时候消费者2进来了，直接就拿了锁（因为生产者释放锁到`wait`函数拿锁这两步不是原子的。存在这种第三方插进来的情况）然后消费了生产的数据。然后释放锁。这时候我们消费者1才终于拿到锁了，但是这个时候抢到的锁并不是被原始生产者释放的锁，而是被其他消费者释放的锁。锁发现数据已经被消费了，这样再去拿数据会有错误。所以必须用`while`。也就是判空。
...
- 针对条件变量和生产者消费者模型到底应该用几个条件变量的问题，一般来说是生产者和消费者各有一个。为什么呢？
  - 假设现在有俩生产者线程和俩消费者线程。四个共用一个条件变量。这时候情况是 1个生产者和俩消费者都在wait。此时单独的抢到锁的生产者生产完毕，这时候notify不一定会notify消费者还是生产者。如果此时notify到了另一个生产者，那么消费者还在等。有可能出现一直是两个生产者在生产，消费者一直没有被notify到的情况。所以还是要消费者和生产者各维护一个比较好。如果是各维护一个，那么生产者notify的一定是消费者。消费者notify的一定是生产者。不会出现循环情况。
