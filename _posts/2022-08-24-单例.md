...
public:
    ...
    {   
        static Student myInstance;              //注意这里，这里是局部静态变量。局部静态变量的生存周期是从声明起至程序结束。 		
...
**类内的函数内的变量是局部变量，不算做类的成员，也不占用类的大小。静态也是这样。而且局部静态变量的生存周期是从声明起至程序结束。在这里就是从函数调用开始到程序结束。所以正是利用了这个特性既保证了线程安全，又是懒汉式，又是全局生存周期。**
