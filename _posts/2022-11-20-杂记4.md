...
- **lambda是一个匿名类，你捕获的变量会变成这个匿名类的类成员变量。**如果按照引用捕获那么这个成员变量就是个引用。自然你在外部修改也会影响里面的值。正因为如此，**如果按照值来捕获，在创建这个lambda表达式的那一刻，被按照值捕获的变量会被在这个匿名类内创建（拷贝）并按照它在这一行之前的那个值来进行附初值。**

- 因为lambda的类型是匿名类，编译器认为每个lambda表达式都不同，哪怕定义完全一样其匿名类名也会不一样，所以lambda表达式只能用auto类型。

- **lambda的默认构造函数和赋值操作默认是delete的。**但是，它有浅拷贝构造函数可被调用。而使用decltype提取未计算的lambda表达式的类型是不允许的，但是，decltype提取已计算的的lambda表达式是可以的。
...
## 关于lambda和智能指针/带初始化器的捕获符相关内容查看智能指针章节。
