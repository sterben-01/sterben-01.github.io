...
- 程序执行前装载器会把用户的参数和环境变量**压入栈而不是放入寄存器**。接着操作系统把控制权交给入口函数。
  ...
  - **入口函数`_start`中调用了`_libc_start_main`函数，`_libc_start_main`函数接受传入参数`_libc_csu_init`函数指针, `__libc_csu_fini`函数指针和`rtld_fini函数指针`**
    ...
    - 在`__libc_start_main`中，我们会调用我们所谓的主函数`main`
      ...
      ![image-20230520224259609](/../sterben-01.github.io/assets/blog_res/2022-11-20-%E6%9D%82%E8%AE%B04.assets/image-20230520224259609.png)
